...
class MCPClient:
    def __init__(self,
        ...
        self.config = config
        self.cwd = cwd
        #will have different for different servers
    ...
    def _create_transport(self)-> SSETransport | StdioTransport:
        if self.config.command:
            ...
            return StdioTransport(
                ...
                env = env,
                cwd = str(self,self.config.cwd or self.cwd),
    ...
    async def connect(self)->None:
        ...
        self.status = MCPServerStatus.CONNECTIONG
        try:
    ...
    async def call_tool(self, tool_name :str, arguments:dict[str,Any]):
        if not self._client or self.status!= MCPServerStatus.CONNECTED:
            raise RuntimeError(f"Not connected to server {self.mane}")
