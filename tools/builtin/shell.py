...
import signal
import sys
from tools.base import Tool, ToolConiformation,ToolInvocation,Toolkind, ToolResult
...
class ShellTool(Tool):
    ...
    async def get_confirmation(self, invocation:ToolInvocation)->ToolConiformation|None:
        ...
        for blocked in BLOCKED_COMMANDS:
            if blocked in command:
                return ToolResult.error_result(
                    f"Command in blocked for safety: {params.command}",
    ...
    def _build_environment(self)->dict[str,str]:
        env = os.environ.copy()
        #Bug:config isnt initialized:initialized in class Tool
